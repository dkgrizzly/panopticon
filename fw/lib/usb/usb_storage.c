...
static int usb_stor_irq(struct usb_device *dev, unsigned long pipe)
{
   struct us_data *us;
...
int usb_stor_BBB_clear_endpt_stall(struct us_data *us, uint8_t endpt)
{
   int result;
            ...
            struct us_data *ss)
{
   ...
   LOG("Endpoints In %d Out %d Int %d\n",
