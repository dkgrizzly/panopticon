...
void _vt100_reset(void)
{
   ...
   p = (uint32_t *) &term.VRam[START_OFFSET];
   cp = &term.ScreenBuf[0];

   while(Len-- > 0) {
      uint32_t cattr = (term.inverse) ? ((term.back_color << 16) | (term.front_color << 8)) : ((term.front_color << 16) | (term.back_color << 8));
      *p++ = cattr | ' ';
      *cp++ = cattr |' ';
   }
...
void _vt100_scroll(struct vt100 *t, int16_t lines)
{
   ...
   if(lines > 0) {
      ...
      // move 4 bytes at a time
         ...
         if((uint32_t *) pTo > &t->ScreenBuf[VT100_WIDTH * VT100_HEIGHT]) {
            ELOG("Past end of ScreenBuf\n");
            for( ; ; );
         }
      }
...
void _vt100_putc(struct vt100 *t, uint8_t ch)
{
   ...
   uint32_t cattr = (t->inverse) ? ((t->back_color << 16) | (t->front_color << 8)) : ((t->front_color << 16) | (t->back_color << 8));
   t->VRam[START_OFFSET + Offset] = cattr | (uint32_t) ch;
